...
import { IDefaultAuthUser, IUnleashConfig } from '../types/option';
...
class UserService {
    ...
    private defaultAuthUser: IDefaultAuthUser
    ...
    constructor(
        stores: Pick<IUnleashStores, 'userStore' | 'eventStore'>,
        {
            ...
            authentication,
            defaultAuthUser,
        }: Pick<IUnleashConfig, 'getLogger' | 'authentication' | 'defaultAuthUser'>,
        services: {
    ...
    async initAdminUser(): Promise<void> {
        ...
        if (userCount === 0) {
            ...
            try {
                const pwd = this.defaultAuthUser.password || 'unleash4all';
                this.logger.info(
                    `Creating default user "admin" with password "${pwd}"`,
                );
                const user = await this.store.insert({
                    username: this.defaultAuthUser.username || 'admin',
                });
